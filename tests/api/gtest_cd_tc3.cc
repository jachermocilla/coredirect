...
class tc_verbs_post_send_en : public verbs_test_cd {};

#define SEND_POST_COUNT		10
#define SEND_EN_WR_ID		((uint64_t)'M')

/* tc_verbs_post_send_en: [TI.1]
 * IBV_EXP_QP_CREATE_CROSS_CHANNEL does not change legacy behaviour
 */
 ...
 * IBV_EXP_WR_SEND_ENABLE, IBV_EXP_WR_RECV_ENABLE, IBV_EXP_WR_CQE_WAIT,
 * IBV_EXP_WR_CALC_SEND
...
TEST_F(tc_verbs_post_send_en, ti_2) {
	...
	rc = __post_write(ctx, TEST_SEND_WRID, IBV_EXP_WR_SEND_ENABLE);
	EXPECT_NE(rc, EOK);
	EXPECT_TRUE((EINVAL == rc) || (EINVAL == errno));

	rc = __post_write(ctx, TEST_SEND_WRID, IBV_EXP_WR_RECV_ENABLE);
	EXPECT_NE(rc, EOK);
	EXPECT_TRUE((EINVAL == rc) || (EINVAL == errno));

	rc = __post_write(ctx, TEST_SEND_WRID, IBV_EXP_WR_CQE_WAIT);
	EXPECT_NE(rc, EOK);
	EXPECT_TRUE((EINVAL == rc) || (EINVAL == errno));

	rc = __post_write(ctx, TEST_SEND_WRID, IBV_EXP_WR_CALC_SEND);
	EXPECT_NE(rc, EOK);
	EXPECT_TRUE((EINVAL == rc) || (EINVAL == errno));
}

/* tc_verbs_post_send_en: [TI.3]
 * IBV_EXP_WR_SEND_ENABLE does not return CQE for QP created
 * w/o IBV_EXP_QP_CREATE_MANAGED_SEND
 */
TEST_F(tc_verbs_post_send_en, ti_3) {
	 ...
	 */
	{
      ...
      //__init_test(IBV_EXP_QP_CREATE_CROSS_CHANNEL | IBV_EXP_QP_CREATE_MANAGED_SEND);
		...
		do {
			if (wrid < SEND_POST_COUNT) {
				rc = __post_write(ctx, wrid, IBV_EXP_WR_SEND_ENABLE);
				ASSERT_NE(EOK, rc);
				++wrid;
			}
 ...
 * IBV_EXP_QP_CREATE_MANAGED_SEND does not affect to the same QP
 */
TEST_F(tc_verbs_post_send_en, ti_5) {
	 ...
	 */
	{
		...
		do {
			if (wrid < SEND_POST_COUNT) {
				rc = __post_write(ctx, wrid, ((wrid % 2) ? IBV_EXP_WR_SEND_ENABLE : IBV_EXP_WR_SEND));
				ASSERT_EQ(EOK, rc);
				++wrid;
			}
		...
		do {
			if (wrid < SEND_POST_COUNT) {
				if (wrid % 2) {
					...
					wr_en.sg_list = NULL;
					wr_en.num_sge = 0;
					wr_en.exp_opcode = IBV_EXP_WR_SEND_ENABLE;
		...
		do {
			if (wrid < SEND_POST_COUNT) {
				...
				if (wrid == (SEND_POST_COUNT - 1)) {
					...
					wr_en[0].num_sge = 0;
					wr_en[0].exp_opcode = IBV_EXP_WR_SEND_ENABLE;
					...
					wr_en[1].num_sge = 0;
					wr_en[1].exp_opcode = IBV_EXP_WR_SEND_ENABLE;
					...
					wr_en[2].num_sge = 0;
					wr_en[2].exp_opcode = IBV_EXP_WR_SEND_ENABLE;
		...
		do {
			if (wrid < SEND_POST_COUNT) {
				...
				if (wrid == (SEND_POST_COUNT - 1)) {
					...
					wr_en[2].num_sge = 0;
					wr_en[2].exp_opcode = IBV_EXP_WR_SEND_ENABLE;
					wr_en[2].exp_send_flags = IBV_SEND_SIGNALED | IBV_EXP_SEND_WAIT_EN_LAST;
					wr_en[2].ex.imm_data = 0;
